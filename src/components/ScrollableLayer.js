    ...
    return (
      <div
        ...
        style={{
          ...
          left: scrollPosition,
          transition:
            isntStatic && this.transition
              ? `left ${this.props.transitionDuration}s ${this.props.transitionEasing} ${this.props.transitionDelay}s`
              : null,
        }}
        onTransitionEnd={() => {
        ...
        {sequence.map((v, i) => {
          ...
          if (layer.included && !layer.ignore) {
            ...
            if (isntStatic) {
              ...
              } else if (i === this.state.to) {
                xPosition = toPosition;
              } else if (this.state.from === -1) {
                xPosition = 0;
              } else {
                hide = true;
              }
            ...
            return (
              <View
                ...
                hidden={
                  !v?.params?.show ||
                  v?.params?.show?.split(/\,\s*/).indexOf(layer.name) < 0
                    ? layer.hiddenNow
                    : false
                }
